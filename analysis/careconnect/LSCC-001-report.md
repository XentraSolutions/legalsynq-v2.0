# LSCC-001 — CareConnect Foundation Stabilization
...
- Appointment-level org scoping is not implemented. Appointments are currently visible to all authenticated users within the tenant. Referrals are the primary cross-org entity; appointment scoping is deferred to LSCC-002 once org-linkage data is populated (all 14 providers currently have no `OrganizationId` set).
...
| `analysis/LSCC-001-report.md` | This report |
...
```
// LSCC-001: CareConnect permission enforcement
...
2. Appointments are not org-scoped in this iteration because `Provider.OrganizationId` is null for all 14 current providers (linkage data not yet populated). Appointment scoping is a follow-up item in LSCC-002.
3. `GET /api/referrals/{id}` does not enforce org-participant visibility at the row level — deferred to LSCC-002.
...
**Mitigation:** Add a data-backfill endpoint or admin UI in LSCC-002 to link providers to Identity organizations.
...
The test project pulls `Microsoft.EntityFrameworkCore 8.0.2` (via BuildingBlocks) and `8.0.8` (via CareConnect.Infrastructure). This causes MSB3277 warnings. Tests pass — the runtime resolves to 8.0.2. Resolved in LSCC-002 by aligning BuildingBlocks to 8.0.8.
...
## 9. Recommended Next Step — LSCC-002

**LSCC-002: CareConnect Org Linkage + Row-Level Access**
