...
public sealed class Bundle : Entity<Guid>
{
    ...
    public static Bundle Create(string key, string name, string? description, DateTime nowUtc)
    {
        return new Bundle
        {
            Id = Guid.NewGuid(),
    ...
    public void Activate(DateTime nowUtc)
    {
        if (Status == CatalogStatus.Retired)
            throw new InvalidOperationException("Retired bundle cannot be activated.");
