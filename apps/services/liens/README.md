...
`PATCH /api/liens/cases/details-update` records every explicit Notes change in case-update history. Notes-only changes appear in `POST /api/liens/cases/case-updates/v3` as action **Case Details Update** with description **Case Tracking Note Update: &lt;submitted note&gt;**; unchanged normalized values do not create duplicates, and clearing an existing Notes value is logged with the generic **Case Tracking Note Update** description. Status aliases are compared by canonical value, preventing display labels such as **In Negotiation** from being recorded as changes when the stored legacy status is equivalent. A changed nonblank value continues to be appended separately to Tracking Notes. Case creation history prefers the authenticated user's email over the token's organization-oriented display name for its **Updated By** value.
`POST /api/liens/cases/liens-updates/v3` excludes case-level **Case Details Update** history without a lien association. Those records remain in `case-updates/v3`; every Liens Updates row represents a specific lien and includes its lien ID, tenant-scoped lien code, and action. Native lien mutations compare persisted previous values with resulting values and list every changed business field as **previous → new**; unchanged submissions do not create history. Summaries longer than the 500-character storage limit continue in additional rows rather than dropping changed fields. Selling handlers that directly mutate a tracked lien apply the same comparison, including archive, restore, and public buyer-response transitions.
`POST /api/liens/cases/liens/update-medical` appends a lien-scoped **Lien Update** row to `liens-updates/v3` only when the submitted note differs from the stored note. It preserves a changed nonblank note as the description, records a cleared note as **Medical note cleared.**, attributes the change to the authenticated user, and does not duplicate an unchanged trimmed note.
...
authenticated updater user ID; the task assignee is not treated as the updater. If
