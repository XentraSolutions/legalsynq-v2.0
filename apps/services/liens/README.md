...
| `GET`, `POST` | `/api/liens/selling/liens/{lienId}/messages` | Authenticated seller message thread for a seller-scoped lien. Reads return every persisted offer-thread message for the lien; sends require an existing buyer offer/access-link thread, persist `senderType=seller`, and notify the buyer through the same offer-message notification workflow as public seller replies. |
