...
public class InvoiceManagementCoreTests
{
    ...
    {
        var (svc, _, _) = Build();
        await Assert.ThrowsAsync<ArgumentException>(() => svc.GetAsync(Guid.Empty, Guid.CreateVersion7()));
    }
    ...
    {
        var (svc, _, _) = Build();
        await Assert.ThrowsAsync<ArgumentException>(() => svc.GetAsync(Guid.CreateVersion7(), Guid.Empty));
    }
