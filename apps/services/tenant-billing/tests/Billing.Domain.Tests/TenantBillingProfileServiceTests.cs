...
public class TenantBillingProfileServiceTests
{
    ...
    public async Task Create_rejects_empty_tenant_or_account()
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>(() =>
            svc.CreateAsync(Guid.Empty, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null));
        await Assert.ThrowsAsync<ArgumentException>(() =>
            svc.CreateAsync(Guid.NewGuid(), Guid.Empty, null, null, TenantBillingMode.InternalOnly, null));
    }
    ...
    public async Task Create_rejects_unknown_mode()
    {
        var (svc, _, _) = Build();
        await Assert.ThrowsAsync<ArgumentException>(() =>
            svc.CreateAsync(Guid.NewGuid(), Guid.NewGuid(), null, null, "Bogus", null));
    }
    ...
    public async Task Create_conflicts_when_tenant_already_has_open_profile()
    {
        var (svc, _, _) = Build();
        var tenant = Guid.NewGuid();
        await svc.CreateAsync(tenant, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);

        await Assert.ThrowsAsync<TenantBillingProfileConflictException>(() =>
            svc.CreateAsync(tenant, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null));
    }
    ...
    public async Task Create_conflicts_when_billing_account_already_claimed_cross_tenant()
    {
        var (svc, _, _) = Build();
        var account = Guid.NewGuid();
        await svc.CreateAsync(Guid.NewGuid(), account, null, null, TenantBillingMode.InternalOnly, null);

        await Assert.ThrowsAsync<TenantBillingProfileConflictException>(() =>
            svc.CreateAsync(Guid.NewGuid(), account, null, null, TenantBillingMode.InternalOnly, null));
    }
    ...
    public async Task Activate_promotes_draft_to_active_and_stamps_activated_at()
    {
        var (svc, _, _) = Build();
        var tenant = Guid.NewGuid();
        var p = await svc.CreateAsync(tenant, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);
    ...
    public async Task Close_allows_creating_new_profile_for_same_tenant()
    {
        var (svc, _, _) = Build();
        var tenant = Guid.NewGuid();
        var first = await svc.CreateAsync(tenant, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);
        await svc.CloseAsync(tenant, first.Id);

        var second = await svc.CreateAsync(tenant, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);
    ...
    public async Task Get_returns_null_for_other_tenant()
    {
        ...
        var p = await svc.CreateAsync(owner, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);
    ...
    public async Task Lifecycle_on_other_tenant_id_is_404()
    {
        ...
        await Assert.ThrowsAsync<TenantBillingProfileNotFoundException>(() =>
            svc.ActivateAsync(Guid.NewGuid(), p.Id));
    }
    ...
    public async Task Resolver_returns_null_for_unknown_or_empty_tenant()
    {
        ...
        (await resolver.GetBillingAccountIdAsync(Guid.NewGuid())).Should().BeNull();
    ...
    public async Task List_pages_only_within_tenant()
    {
        ...
        var t1 = Guid.NewGuid();
        var t2 = Guid.NewGuid();
        await svc.CreateAsync(t1, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);
        await svc.CreateAsync(t2, Guid.NewGuid(), null, null, TenantBillingMode.InternalOnly, null);
