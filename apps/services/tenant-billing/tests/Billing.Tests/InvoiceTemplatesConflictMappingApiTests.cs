...
public class InvoiceTemplatesConflictMappingApiTests : IDisposable
{
    ...
    /// <see cref="BillingWebApplicationFactory"/> so the rest of
    ...
    private sealed class ConflictThrowingFactory : WebApplicationFactory<Program>
    {
        ...
        protected override void ConfigureWebHost(IWebHostBuilder builder)
        {
            ...
            builder.ConfigureAppConfiguration((_, config) =>
            {
                config.AddInMemoryCollection(new Dictionary<string, string?>
                {
                    ...
                    // TB-MERGE-02 fix: this factory was a clone of
                    // BillingWebApplicationFactory but had drifted and was
                    // missing the internal-token + platform-template
                    // configuration the runtime middleware enforces. Without
                    // them every /api/* request short-circuited at the
                    // RequireInternalTokenMiddleware with 401, so the
                    // controller's 409-mapping branch was never reached.
                    [RequireInternalTokenMiddleware.ConfigurationKey] =
                        BillingWebApplicationFactory.TestInternalToken,
        ...
        // TB-MERGE-02 fix: pre-populate the internal-token header on every
        // client returned from this factory, mirroring
        // BillingWebApplicationFactory.ConfigureClient.
