...
public interface ICustomerRepository
{
    ...
    /// <summary>
    /// Tenant-scoped EXACT lookup by <see cref="Customer.ExternalReference"/>.
    /// Match is case-insensitive on the trimmed needle. Excludes soft-deleted
    /// rows.
    ///
    /// Returns at most <paramref name="limit"/> matches so callers can
    /// distinguish "not found" (0) from "unique match" (1) from "ambiguous /
    /// duplicate" (2+) without first counting the entire table. <c>limit</c>
    /// is clamped to <c>[1, 10]</c>; the default of 2 is sufficient for the
    /// MS-BILL-UI-017 BFF resolver use case (uniqueness check).
    /// </summary>
    Task<IReadOnlyList<Customer>> GetByExternalReferenceAsync(
        Guid tenantId,
