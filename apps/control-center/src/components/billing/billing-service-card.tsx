import type { BillingServiceStatus } from '@/types/control-center';

interface BillingServiceCardProps {
  status:     BillingServiceStatus;
  latencyMs?: number;
  checkedAt:  string;
}

const STATUS_CONFIG: Record<BillingServiceStatus, {
  bg:    string;
  ring:  string;
  dot:   string;
  text:  string;
  ...
  online:   { bg: 'bg-green-50', ring: 'ring-green-200', dot: 'bg-green-500', text: 'text-green-700', label: 'Tenant Billing Service Online'  },
  degraded: { bg: 'bg-amber-50', ring: 'ring-amber-200', dot: 'bg-amber-500', text: 'text-amber-700', label: 'Tenant Billing Service Degraded' },
  offline:  { bg: 'bg-red-50',   ring: 'ring-red-200',   dot: 'bg-red-600',   text: 'text-red-700',   label: 'Tenant Billing Service Offline'  },
};

export function BillingServiceCard({ status, latencyMs, checkedAt }: BillingServiceCardProps) {
