...
export default async function ApplicationDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <CCShell userEmail={session.email}><div className="mx-auto max-w-[1160px] space-y-6">
    ...
    <InfoCard title="Subdomain & Provisioning"><div className="grid gap-5 md:grid-cols-4"><Field label="STATUS" value={formatProvisioningStatus(application.provisioningStatus)}/><Field label="HOSTNAME" value={application.provisioningHostname || '—'}/><Field label="FAILURE STAGE" value={application.provisioningFailureStage || '—'}/><Field label="REASON" value={application.provisioningError || '—'}/></div></InfoCard>
...
function formatProvisioningStatus(value: string) { return value === 'InProgress' ? 'In Progress' : value || 'Not Started'; }
