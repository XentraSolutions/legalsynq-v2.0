...
describe("MyNetworkClient", () => {
  ...
  test("requires a specialty before creating a provider and submits selected specialty codes", async () => {
    const user = userEvent.setup();
    vi.mocked(careConnectApi.networks.addProvider).mockResolvedValue(
      ok({
        ...BASE_PROVIDER,
    ...
    await user.click(
      screen.getByRole("button", { name: /Not found\? Add new instead/i }),
    );
    ...
    await user.type(
      screen.getByPlaceholderText("(555) 555-5555"),
      "5555555555",
    );
    ...
    await user.click(
      screen.getByRole("button", { name: /Add to Registry & My Network/i }),
    );

    expect(
      await screen.findAllByText("Select at least one specialty."),
    ).not.toHaveLength(0);
    expect(careConnectApi.networks.addProvider).not.toHaveBeenCalled();

    await user.click(
      screen.getByRole("checkbox", { name: "Physical Therapy" }),
    );
    await user.click(
      screen.getByRole("button", { name: /Add to Registry & My Network/i }),
    );
    ...
    await waitFor(() =>
      expect(careConnectApi.networks.addProvider).toHaveBeenCalledTimes(1),
    );
  ...
  test("adds a new location for an existing provider through the explicit search flow", async () => {
    const user = userEvent.setup();
    vi.mocked(careConnectApi.networks.searchProviders).mockResolvedValue(
      ok([BASE_SEARCH_RESULT]),
    );
    vi.mocked(careConnectApi.networks.addProvider).mockResolvedValue(
      ok({
        ...BASE_PROVIDER,
    ...
    expect(
      screen.queryByText("Location / Facility name *"),
    ).not.toBeInTheDocument();
    const locationForm = screen
      .getByRole("button", { name: /Add Location to My Network/i })
      .closest("form") as HTMLFormElement;
    // Organization/Practice, Email, and Phone come from the existing provider record and are display-only here.
    expect(
      within(locationForm).queryByRole("textbox", {
        name: /Organization \/ Practice/i,
      }),
    ).not.toBeInTheDocument();
    expect(
      within(locationForm).getByText("Smith Family Practice"),
    ).toBeInTheDocument();
    expect(
      within(locationForm).getByText(BASE_SEARCH_RESULT.email),
    ).toBeInTheDocument();
    expect(
      within(locationForm).getByText("(555) 123-4567"),
    ).toBeInTheDocument();
    ...
    await user.click(
      screen.getByRole("button", { name: /Add Location to My Network/i }),
    );
    ...
    const request = vi.mocked(careConnectApi.networks.addProvider).mock
      .calls[0]?.[1];
  ...
  test("shows every facility returned by provider registry search", async () => {
    ...
    expect(
      screen.getByText(/120 Green Street Greenland, AR 72701/),
    ).toBeInTheDocument();
    expect(
      screen.getByText(/120 Market Street San Francisco, CA 94111/),
    ).toBeInTheDocument();
  });

  test("requires a specialty before saving provider setup and submits grouped setup fields", async () => {
    const user = userEvent.setup();
    vi.mocked(careConnectApi.networks.updateProvider).mockResolvedValue(
      ok({
        ...BASE_PROVIDER,
    ...
    render(
      <MyNetworkClient
        initialNetwork={makeNetwork([
          { ...BASE_PROVIDER, name: "Dr. Atlas Rehab", title: "Dr." },
        ])}
        fetchError={null}
        specialtyOptions={SPECIALTIES}
        tenantName="Test Tenant"
    ...
    expect(
      screen.getByRole("button", { name: /Save Provider Setup/i }),
    ).toBeInTheDocument();
    ...
    await waitFor(() =>
      expect(careConnectApi.networks.updateProvider).toHaveBeenCalledTimes(1),
    );
    ...
    expect(
      vi.mocked(careConnectApi.networks.updateProvider).mock.calls[0]?.[2]
        .facilityName,
    ).toBeUndefined();
  });
  ...
  test("shows all provider locations in edit mode and soft deletes one location", async () => {
    const user = userEvent.setup();
    vi.mocked(careConnectApi.networks.removeProvider).mockResolvedValue(
      ok(undefined),
    );
    ...
    vi.mocked(careConnectApi.networks.updateProvider).mockImplementation(
      async (_networkId, entryId, request) =>
        ok({
    ...
    await waitFor(() =>
      expect(careConnectApi.networks.updateProvider).toHaveBeenCalledTimes(2),
    );
    ...
    await user.click(
      within(southForm as HTMLFormElement).getByRole("button", {
    ...
    expect(
      within(dialog).getByText(/Atlas Health - South/),
    ).toBeInTheDocument();
    await user.click(
      within(dialog).getByRole("button", { name: /Delete location/i }),
    );

    await waitFor(() =>
      expect(careConnectApi.networks.removeProvider).toHaveBeenCalledTimes(1),
    );
    ...
    expect(careConnectApi.networks.removeProvider).toHaveBeenCalledWith(
      "network-1",
      "network-provider-2",
      true,
    );
    ...
    await waitFor(() =>
      expect(screen.queryByDisplayValue("Dallas")).not.toBeInTheDocument(),
    );
  ...
  test('"Remove from network" icon uses the styled ConfirmDialog, no Facility cascade, updated toast wording', async () => {
    ...
    expect(
      await screen.findByText("Atlas Rehab set to inactive."),
    ).toBeInTheDocument();
    await waitFor(() =>
      expect(screen.queryByRole("dialog")).not.toBeInTheDocument(),
    );
  ...
  test("submits backend geo point source after selecting a geocoded address", async () => {
    ...
    await user.click(
      await screen.findByText("123 North Usa Drive, Greenland, AR 72701"),
    );
    await user.click(
      screen.getByRole("checkbox", { name: "Physical Therapy" }),
    );
  ...
  test("Edit Provider existing location selects a geocoded address and submits its coordinates", async () => {
    const user = userEvent.setup();
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ...
        json: async () => [
          {
            displayName: "456 Oak Street, Austin, TX 78702",
    ...
    render(
      <MyNetworkClient
        initialNetwork={makeNetwork([
          { ...BASE_PROVIDER, specialties: [SPECIALTIES[0]] },
        ])}
        fetchError={null}
        specialtyOptions={SPECIALTIES}
        tenantName="Test Tenant"
    ...
    await user.click(screen.getByTitle("Edit provider"));
    const address = screen.getByDisplayValue("123 Main St");
    await user.clear(address);
    await user.type(address, "456 Oak Street");
    await user.click(
      await screen.findByText("456 Oak Street, Austin, TX 78702"),
    );

    expect(screen.getByDisplayValue("456 Oak Street")).toBeInTheDocument();
    expect(screen.getByDisplayValue("78702")).toBeInTheDocument();
    await user.click(screen.getByRole("button", { name: /Save Location/i }));
  ...
  test("Edit Provider New location selects a geocoded address and submits its coordinates", async () => {
    const user = userEvent.setup();
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
        ...
        json: async () => [
          {
            displayName: "789 South Street, Dallas, TX 75201",
            addressLine1: "789 South Street",
            city: "Dallas",
            state: "TX",
            postalCode: "75201",
            latitude: 32.7767,
    ...
    const newLocation = screen
      .getByText("New location")
      .closest("form") as HTMLFormElement;
    const address = within(newLocation).getByPlaceholderText("123 Main St");
    await user.type(address, "789 South Street");
    await user.click(
      await within(newLocation).findByText(
        "789 South Street, Dallas, TX 75201",
      ),
    );
    await user.click(
      within(newLocation).getByRole("button", { name: /Save Location/i }),
    );
  ...
  test("Edit Provider blocks saving when the ZIP no longer matches the selected address", async () => {
    const user = userEvent.setup();
    vi.stubGlobal(
      "fetch",
      vi.fn().mockResolvedValue({
    ...
    const zip = screen.getByDisplayValue("78702");
    await user.clear(zip);
    await user.type(zip, "78703");
    await user.click(screen.getByRole("button", { name: /Save Location/i }));

    expect(careConnectApi.networks.updateProvider).not.toHaveBeenCalled();
    expect(
      await screen.findByText("ZIP code does not match the selected address."),
    ).toBeInTheDocument();
  });
  ...
  test("adds a new location from the edit-provider panel and returns to the edit view", async () => {
    ...
    const newLocationCard = screen
      .getByText("New location")
      .closest("form") as HTMLFormElement;
    expect(
      within(newLocationCard).queryByRole("textbox", {
        name: /Organization \/ Practice/i,
      }),
    ).not.toBeInTheDocument();

    const newLocationCity = labelByText(
      "City *",
      newLocationCard,
    ).parentElement?.querySelector("input") as HTMLInputElement;
    await user.type(
      within(newLocationCard).getByPlaceholderText("123 Main St"),
      "789 South St",
    );
    await user.type(newLocationCity, "Dallas");
    await user.type(within(newLocationCard).getByPlaceholderText("IL"), "TX");
    await user.type(
      within(newLocationCard).getByPlaceholderText("60601"),
      "75201",
    );
    await user.click(
      within(newLocationCard).getByRole("button", { name: /Save Location/i }),
    );
  ...
  test('LSV3-1084: shows "{tenantName} Preferred Providers" heading and no description', () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Acme Law Preferred Providers" }),
    ).toBeInTheDocument();
  ...
  test("LSV3-1213: supports Network Setup heading override", () => {
    ...
    expect(
      screen.getByRole("heading", { name: "Network Setup" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("heading", { name: "Acme Law Preferred Providers" }),
    ).not.toBeInTheDocument();
  });
  ...
  test("LSV3-1213: Network Setup can show only the caller organization's providers", () => {
    ...
    expect(screen.getByText("Atlas Rehab")).toBeInTheDocument();
    expect(
      screen.queryByText("Other Network Provider"),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Tenant Network Provider"),
    ).not.toBeInTheDocument();
