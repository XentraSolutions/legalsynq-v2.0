...
export function TaskPanel({
  ...
  useEffect(() => {
    apiClient
      ...
      .catch(() => {
        /* best-effort — cards fall back to icon */
      });
  }, []);
  ...
  const counts = {
    active: tasks.filter((t) =>
      ACTIVE_TASK_STATUSES.includes(t.status as never),
    ).length,
    all: tasks.length,
  };
          ...
          {filteredTasks.map((task) => (
            <TaskCard
              ...
              onDelete={async (t) => {
                ...
                try {
                  await lienTasksService.deleteTask(t.id);
                  // refresh tasks after successful delete
                  await fetchTasks();
                  // clear any open selection or edit state for the deleted task
                  setSelectedTask((prev) => (prev?.id === t.id ? null : prev));
                  setEditTask((prev) => (prev?.id === t.id ? undefined : prev));
                } finally {
