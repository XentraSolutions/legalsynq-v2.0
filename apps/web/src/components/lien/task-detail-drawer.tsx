...
export function TaskDetailDrawer({
  ...
  const [activeTab, setActiveTab] = useState<"notes" | "details" | "history">(
    "notes",
  );
  ...
  async function handleStatusChange(newStatus: TaskStatus) {
    ...
    setStatusError(null);
    try {
      const updated = await lienTasksService.updateStatus(
        localTask.id,
        newStatus,
      );
  ...
  async function handleAddNote() {
    ...
    setSubmitting(true);
    try {
      const note = await lienTaskNotesService.createNote(
        task.id,
        draftText.trim(),
      );
  ...
  async function handleSaveEdit(noteId: string) {
    ...
    setSavingEdit(true);
    try {
      const updated = await lienTaskNotesService.updateNote(
        task.id,
        noteId,
        editingText.trim(),
      );
      ...
      >
        ...
        <div className="flex items-start justify-between px-6 py-4 border-b border-gray-100 flex-shrink-0">
          <div className="flex-1 min-w-0 pr-4">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span
                className={`inline-flex text-xs font-medium px-2 py-0.5 rounded-full ${statusCfg.bg} ${statusCfg.text}`}
              >
        ...
        <div className="px-6 py-3 border-b border-gray-100 flex-shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            {statusChanging === null && (
              <span
                className={`inline-flex items-center gap-1 text-xs font-medium px-2.5 py-1.5 rounded-full ${TASK_STATUS_COLORS[displayTask.status].bg} ${TASK_STATUS_COLORS[displayTask.status].text}`}
              >
        ...
        <div className="px-6 py-3 border-b border-gray-100 flex-shrink-0">
          <div className="grid grid-cols-2 gap-3 text-xs text-gray-500">
            <div className="flex items-center gap-1.5">
              <i
                className={`${TASK_PRIORITY_ICONS[displayTask.priority]} ${TASK_PRIORITY_COLORS[displayTask.priority]}`}
              />
              <span className="font-medium text-gray-700">
                {displayTask.priority.charAt(0) +
                  displayTask.priority.slice(1).toLowerCase()}{" "}
                priority
              </span>
            </div>
            {displayTask.dueDate && (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  Due <DateDisplay value={displayTask.dueDate} format="date" />
                </span>
              </div>
            )}
            {displayTask?.linkedLiens?.length > 0 && (
              <div className="flex items-center gap-1.5">
                ...
                <span>
                  {displayTask.linkedLiens.length} lien
                  {displayTask.linkedLiens.length !== 1 ? "s" : ""} linked
                </span>
              </div>
            )}
          </div>
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              {historyLoaded && historyEvents.length > 0 && (
                <span className="ml-1 bg-gray-100 text-gray-500 text-xs rounded-full px-1.5 py-0.5 font-semibold">
    ...
    <div className="space-y-4">
      <div className="bg-violet-50 border border-violet-200 rounded-xl p-4">
        ...
        <p className="text-xs text-violet-700 leading-relaxed">
          This task was generated automatically by the LegalSynq rules engine
          based on a configured trigger and template.
        </p>
      </div>
      ...
      <div className="rounded-xl border border-gray-100 bg-gray-50 p-4 space-y-2">
        ...
        <div className="flex gap-2 text-xs text-gray-600">
          <span className="flex-shrink-0 w-5 h-5 rounded-full bg-violet-100 text-violet-600 font-bold flex items-center justify-center text-[10px]">
            1
          </span>
          <span>
            A domain event (e.g. lien status change, case stage update) was
        ...
        <div className="flex gap-2 text-xs text-gray-600">
          ...
          <span>
            The matching generation rule evaluated the event and found a
            configured template.
          </span>
        </div>
        <div className="flex gap-2 text-xs text-gray-600">
          ...
          <span>
            This task was created from the template, pre-filled with the
  ...
  return (
    <div className="space-y-3">
      ...
      <DetailRow
        ...
        label="Last updated"
        value={formatDateTime(task.updatedAtUtc)}
      />
      {task.completedAt && (
        <DetailRow
      ...
      {task.workflowStageId && (
        <DetailRow
          ...
          value={task.workflowStageId}
          mono
        />
      )}
      {task.workflowInstanceId && (
        <LinkedWorkflowSection
          instanceId={task.workflowInstanceId}
          stepKey={task.workflowStepKey}
        />
      )}
    </div>
  );
...
function LinkedWorkflowSection({
  instanceId,
  stepKey,
}: {
  instanceId: string;
  ...
  return (
    <div className="flex items-start gap-3 py-2 border-b border-gray-50 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p
          className={`text-sm text-gray-700 break-all ${mono ? "font-mono text-xs" : ""} ${valueClass}`}
        >
          {value}
        </p>
      </div>
...
const EVENT_STYLES: Record<
  string,
  { icon: string; dot: string; badge: string }
> = {
  TASK_CREATED: {
...
function parseStatusFromDescription(
  description: string,
      ...
      <div className="relative">
        ...
        <div className="space-y-0">
          {events.map((event, idx) => {
            ...
            return (
              <div
                key={event.auditId ?? idx}
                className="relative flex gap-4 pb-6 last:pb-0"
              >
                {/* Dot */}
                <div
                  className={`relative z-10 w-6 h-6 rounded-full ${style.dot} flex items-center justify-center flex-shrink-0 mt-0.5 shadow-sm`}
                >
                ...
                <div className="flex-1 min-w-0 pt-0.5">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span
                      className={`inline-flex items-center text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full ${style.badge}`}
                    >
                      {friendlyLabel(event.eventType)}
                    </span>
                    <span className="text-xs text-gray-400 ml-auto whitespace-nowrap">
                      <DateDisplay
                        value={event.occurredAtUtc}
                        format="datetime"
                      />
                    </span>
                  </div>
