...
function deriveHistory(lien: LienDetail): LienStatusHistoryItem[] {
  const items: LienStatusHistoryItem[] = [
    {
      status: "Draft",
      occurredAtUtc: lien.createdAtUtc,
      label: "Lien created (Draft)",
      actorOrgName: lien.sellingOrg?.orgName,
    },
  ];
  ...
  if (lien.status === "Offered" || lien.status === "Sold") {
    items.push({
      status: "Offered",
      occurredAtUtc: lien.updatedAtUtc,
  ...
  if (lien.status === "Sold") {
    items.push({
      ...
      occurredAtUtc: lien.updatedAtUtc,
      label: "Purchased",
      actorOrgName: lien.buyingOrg?.orgName,
    });
  }
  if (lien.status === "Withdrawn") {
    items.push({
      status: "Withdrawn",
      occurredAtUtc: lien.updatedAtUtc,
      label: "Withdrawn from marketplace",
      actorOrgName: lien.sellingOrg?.orgName,
    });
  }
    ...
    <div className="bg-white border border-gray-200 rounded-lg">
      ...
      <div className="px-6 py-5 border-b border-gray-100 flex items-start justify-between gap-4">
        <div>
          ...
          {lien.jurisdiction && (
            <p className="text-sm text-gray-500 mt-0.5">{lien.jurisdiction}</p>
          )}
        </div>
        ...
        <Section title="Financial details">
          <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
            <Field
              ...
              value={formatCurrency(lien.originalAmount)}
            />
            <Field
              label="Offer price"
              value={formatCurrency(lien.offerPrice)}
            />
              ...
              <Field
                ...
                value={
                  <span className="text-green-700 font-semibold">
                    {formatCurrency(lien.purchasePrice)}
                  </span>
                }
              />
            )}
        ...
        <Section title="Organisations">
          <dl className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-5">
            ...
            <Field
              label="Buying org"
              value={
                lien.buyingOrg?.orgName ??
                (lien.status !== "Sold" ? "Not yet purchased" : "—")
              }
            />
        ...
        {lien.isConfidential && lien.subjectParty && (
          <Section title="Subject party">
            <p className="text-sm italic text-yellow-700">
              Subject identity is confidential and not disclosed until after
              purchase.
            </p>
          </Section>
        )}
