...
export default function ServicingDetailPage() {
  ...
  const fetchItem = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load servicing task",
      );
    } finally {
      setLoading(false);
    }
  ...
  async function handleStatusUpdate(status: string) {
    ...
    try {
      const updated = await servicingService.updateStatus(item.caseId, status);
      setItem(updated);
      addToast({
        type: "success",
    ...
    <div className="space-y-5">
            ...
            <div className="space-y-3 text-sm">
              ...
              {item.startedAt && (
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-gray-700">{item.startedAt}</span>
                </div>
              )}
              {item.escalatedAt && (
                <div className="flex items-center justify-between">
                  <span className="text-red-400">Escalated</span>
                  <span className="text-red-700">{item.escalatedAt}</span>
                </div>
              )}
              {item.completedAt && (
                <div className="flex items-center justify-between">
                  <span className="text-green-400">Completed</span>
                  <span className="text-green-700">{item.completedAt}</span>
                </div>
              )}
              <div className="flex items-center justify-between">
                <span className="text-gray-400">Updated</span>
                <span className="text-gray-700">{item.updatedAt}</span>
              </div>
            </div>
          </div>
      ...
      {confirmAction && (
        <ConfirmDialog
          ...
          onConfirm={async () => {
            await handleStatusUpdate(confirmAction.status);
