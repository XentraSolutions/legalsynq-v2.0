...
export function LienDetailClient({ id }: { id: string }) {
  ...
  const personName =
    d.subjectName || `${d.subjectFirstName} ${d.subjectLastName}`.trim();
  ...
  const handleSubmitOffer = async () => {
    ...
    try {
      ...
      addToast({
        ...
        description: `$${Number(offerAmount).toLocaleString()} offer placed`,
      });
      setOfferAmount("");
      setOfferNotes("");
      ...
      const message =
        err instanceof ApiError ? err.message : "Failed to submit offer";
  ...
  const handleAcceptOffer = async (offerId: string) => {
    try {
      ...
      addToast({
        ...
        description: `Lien sold — Bill of Sale ${result.billOfSaleNumber} created`,
      });
      setConfirmAction(null);
      await fetchLien();
    } catch (err) {
      const message =
        err instanceof ApiError ? err.message : "Failed to accept offer";
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-auto bg-gray-50 px-6 py-5">
        ...
        {activeTab === "notes" && (
          <NotesPanel notes={lienNotes} onAddNote={() => {}} readOnly />
        )}
        {activeTab === "history" && (
      ...
      <FormModal
        open={isSellMode && showOfferModal}
        onClose={() => setShowOfferModal(false)}
        onSubmit={handleSubmitOffer}
        title="Submit Offer"
        submitLabel={submitting ? "Submitting..." : "Submit Offer"}
        submitDisabled={
          !offerAmount || isNaN(Number(offerAmount)) || submitting
        }
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Offer Amount<span className="text-red-500 ml-0.5">*</span>
            </label>
    ...
    <div className="space-y-4">
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
        <div className="px-5 py-4">
          ...
          <StatusProgress
            steps={isSellMode ? SELL_LIEN_STEPS : MANAGE_LIEN_STEPS}
            currentStep={STATUS_MAP[d.status] || "Draft"}
          />
        </div>
      </div>
        ...
        <FieldGrid>
          ...
          <FieldItem
            ...
            value={
              linkedCase ? (
                <Link
                  href={`/lien/cases/${d.caseId}`}
                  className="text-primary hover:underline text-sm"
                >
                  {linkedCase.caseNumber} — {linkedCase.clientName}
                </Link>
              ) : d.caseId ? (
          ...
          <FieldItem
            ...
            value={
              d.isConfidential
                ? "Confidential"
                : d.subjectName ||
                  `${d.subjectFirstName} ${d.subjectLastName}`.trim() ||
                  undefined
            }
          />
          <FieldItem
            label="First Name"
            value={d.isConfidential ? "Confidential" : d.subjectFirstName}
          />
          <FieldItem
            label="Last Name"
            value={d.isConfidential ? "Confidential" : d.subjectLastName}
          />
        </FieldGrid>
      </CollapsibleSection>
              ...
              >
                <div>
                  <p className="text-sm text-gray-700 font-medium">
                    Offer from Org {offer.buyerOrgId.slice(0, 8)}...
                  </p>
                  <p className="text-xs text-gray-400">
                    {offer.notes || "No notes"} &middot; {offer.offeredAt}
                  </p>
                </div>
      ...
      {isSellMode && pendingOffers.length > 0 && (
        <div className="flex items-center gap-2 px-4 py-3 bg-amber-50 border border-amber-200 rounded-lg">
          ...
          <p className="text-xs text-amber-700">
            <span className="font-medium">Action Required:</span> This lien has{" "}
            {pendingOffers.length} pending offer(s) requiring review.
          </p>
        </div>
      )}
    </div>
  ...
  const rightContent = (
    <div className="space-y-4">
      ...
      <CollapsibleSection title="Communications" icon="ri-mail-line">
        ...
        <div className="space-y-4">
          <div>
            <div className="text-[11px] font-medium text-gray-400 uppercase tracking-wide leading-tight mb-2.5">
              Contacts
            </div>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-10 text-center">
      ...
      <p className="text-sm text-gray-400 mt-2">
        {message || `No ${label.toLowerCase()} data available`}
      </p>
    </div>
  );
}
