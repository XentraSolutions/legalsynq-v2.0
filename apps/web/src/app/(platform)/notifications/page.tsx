...
function ChannelBadge({ channel }: { channel: string }) {
  const cls =
    CHANNEL_CLS[channel.toLowerCase()] ??
    "bg-gray-100 text-gray-500 border border-gray-200";
  return (
    <span
      className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium capitalize ${cls}`}
    >
      {channel}
    </span>
  );
...
export default async function NotificationsPage() {
  ...
  const tzResult = await tenantServerApi
    .getTimezoneSetting(tenantId)
    .catch(() => null);
      ...
      {statsError ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          ...
          <StatCard
            ...
            note={
              stats
                ? `${stats.sentCount} of ${stats.totalCount} delivered`
                : undefined
            }
            accent={
              deliveryRate !== null
          ...
          <StatCard
            ...
            note={
              stats
                ? `${last24h.sent} sent · ${last24h.failed} failed`
                : undefined
            }
          />
          <StatCard
            ...
            note={
              stats
                ? `${last7d.sent} sent · ${last7d.failed} failed`
                : undefined
            }
          />
        </div>
      ...
      {stats && stats.totalCount > 0 && (
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
            {(
              ["sent", "accepted", "processing", "failed", "blocked"] as const
            ).map((s) => (
