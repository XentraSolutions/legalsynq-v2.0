...
export const OffersApi = {
  async listOffers(params: OfferQueryParams): Promise<PagedResult<Offer>> {
    const response = await apiClient.get<PagedResult<Offer>>('/liens/offers', { params });
    return response.data;
  },

  async getOffer(id: string): Promise<Offer> {
    const response = await apiClient.get<Offer>(`/liens/offers/${id}`);
    return response.data;
  },

  async acceptOffer(id: string, body: OfferActionRequest = {}): Promise<Offer> {
    const response = await apiClient.patch<Offer>(`/liens/offers/${id}/accept`, body);
    return response.data;
  },

  async declineOffer(id: string, body: OfferActionRequest = {}): Promise<Offer> {
    const response = await apiClient.patch<Offer>(`/liens/offers/${id}/decline`, body);
    return response.data;
  },
};
