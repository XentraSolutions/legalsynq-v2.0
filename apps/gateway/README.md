...
- Deep-link association files are deterministic JSON artifacts generated by `scripts/deep-links/generate-association-files.py` from approved environment values. Phase-1 OS association scope is explicitly `portal-root`: AASA claims exact `/` independently of the shared resource-route registry. The gateway serves only generated files; it does not calculate app identities, read signing secrets, or call business services.
...
The required explicit scope prevents an old caller from silently receiving different AASA semantics. It does not affect `assetlinks.json`: Android path narrowing remains in Mobile's exact-root intent filter.
...
The config must continue to supply the externally approved Apple Team ID/bundle ID and Android package/public installed-build SHA-256 fingerprints. Generation does not approve those values, deploy an artifact, or prove physical-device handoff.
