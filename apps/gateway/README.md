...
- Deep-link association files are deterministic JSON artifacts generated by `scripts/deep-links/generate-association-files.py` from approved environment values and the read-only shared route registry. The association tooling consumes each registry route's `pathTemplate` field. The gateway serves only the generated files; it does not calculate app identities, read signing secrets, or call business services.
