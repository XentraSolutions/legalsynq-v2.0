...
public interface IReportScheduleService
{
    ...
    Task<ServiceResult<ReportScheduleResponse>> DeactivateScheduleAsync(Guid scheduleId, string userId, CancellationToken ct = default);
    Task<ServiceResult<ReportScheduleRunResponse>> TriggerRunNowAsync(Guid scheduleId, string userId, CancellationToken ct = default);
