-- Soft-deletes the reviewed malformed imported payment for Hector Zaldana,
-- case 26-31912, lien 26-31912-01. The SL-CORE No Recovery declaration has
-- no receipt evidence, but its $17,228 amount-to-settle was imported as cash.
-- Both liens are already open and remain byte-for-byte unchanged.
--
...
DROP TEMPORARY TABLE IF EXISTS tmp_hector_expected_crosswalks;
CREATE TEMPORARY TABLE tmp_hector_expected_crosswalks (
...
INSERT INTO tmp_hector_expected_crosswalks
...
FROM tmp_hector_expected_crosswalks expected
...
SET @plan_checksum = SHA2(
    CONCAT_WS(
        ...
        @lien_01_id, @lien_02_id, @payment_id, @legacy_payment_id,
        @import_run_id,
        '196e4df2b292728cdb8159b5a338dd01f3ab3683c638624f3e75362d87212a5a',
        'b0a8b3a76990e504075ed9dd79e08665c5cbea57015899021bacb151d08a0e34',
        '1a368bb1965a23502eb10e347912cf5126dfba76e701c14e29edcebf4ccedee4',
        ...
        'PreDemand', '32078.00', 'NULL', 'Active', 'Active',
        '17228.00', '17228.00', '4307.00',
        '14850.00', '14850.00', '3712.50',
        '1', '17228.00', 'NULL', 'NULL', 'NULL',
        '2026-07-16 21:47:24.000000',
        '2026-07-16 00:00:00.000000',
        @payment_note
    ),
    256
);
...
SET @locked_preimages = IF(
    ...
    AND @locked_active_payment_total = 17228.00
    ...
    AND @locked_initial_payment_ok = 1,
    1,
    0
);
...
SET @postcondition_errors = IF(
    ...
    AND @rows_updated = 1
    AND @post_active_payment_count = 0
    AND @post_active_payment_total = 0.00
    AND @post_active_no_recovery_count = 0
...
PREPARE hector_repair_conditional_commit FROM @conditional_commit_sql;
EXECUTE hector_repair_conditional_commit;
DEALLOCATE PREPARE hector_repair_conditional_commit;
ROLLBACK;
...
SELECT
    ...
    CASE
        ...
        ELSE 'Applied successfully: false receipt removed; both liens remain open.'
    END AS Result;
