-- One-time QA approval for Program 1 into tenant 019f6ae6-4348-784a-aae0-f4d636f843ad.
--
-- Run with DBeaver "Execute SQL Script" while connected to the same MySQL
-- server as LS_QA_LIENS. Autocommit must be enabled. The supplied QA Identity
-- dump was used only to identify the active lien-owner organization and the
-- sole active TenantAdmin below; every value is revalidated before insertion.
--
-- REQUIRED: choose exactly one approved amount source before executing:
--   SET @lien_amount_source = 'billing';
-- or
--   SET @lien_amount_source = 'purchase';
--
-- A single import cannot store both source amounts in the target's one
-- OriginalAmount/CurrentBalance pair. This script inserts no row while the
-- amount source is NULL or invalid.

USE LS_QA_LIENS;

SET @tenant_id = '019f6ae6-4348-784a-aae0-f4d636f843ad';
SET @org_id = '019f6ae6-451a-75ef-8e7c-ed3a4eb57620';
SET @migration_user_id = '019f6ae6-451b-774f-8c02-e45b704dc655';
SET @approved_by_user_id = '019f6ae6-451b-774f-8c02-e45b704dc655';
SET @lien_amount_source = NULL; -- Change to exactly 'billing' or 'purchase'.
...
SELECT
  EXISTS (
    SELECT 1
    FROM LS_QA_IDENTITY.idt_Organizations o
    WHERE o.Id = @org_id
  ...
  EXISTS (
    SELECT 1
    FROM LS_QA_IDENTITY.idt_Users u
    INNER JOIN LS_QA_IDENTITY.idt_UserTenants ut
      ON ut.UserId = u.Id
  ...
  EXISTS (
    ...
    INNER JOIN LS_QA_IDENTITY.idt_UserTenants ut
     ...
     AND ut.TenantId = @tenant_id
     AND ut.IsActive = 1
    INNER JOIN LS_QA_IDENTITY.idt_ScopedRoleAssignments sra
      ON sra.UserId = u.Id
     AND sra.IsActive = 1
    INNER JOIN LS_QA_IDENTITY.idt_Roles r
      ON r.Id = sra.RoleId
  ...
  EXISTS (
    SELECT 1
    FROM LS_QA_IDENTITY.idt_TenantProductEntitlements entitlement
