...
BEGIN
    ...
    DECLARE v_settlement_count     INT DEFAULT 0;
    DECLARE v_payment_detail_count INT DEFAULT 0;
    DECLARE v_total_billing        DECIMAL(20,4) DEFAULT 0;
    DECLARE v_total_purchase       DECIMAL(20,4) DEFAULT 0;
    DECLARE v_total_cash_received  DECIMAL(20,4) DEFAULT 0;
    -- Applied row counts
    ...
    DECLARE v_settlements_ins      INT DEFAULT 0;
    DECLARE v_payment_details_ins  INT DEFAULT 0;
    DECLARE v_postcondition_errors INT DEFAULT 0;

    DECLARE EXIT HANDLER FOR SQLEXCEPTION
    BEGIN
        IF v_in_transaction THEN ROLLBACK; SET v_in_transaction = FALSE; END IF;
        DROP TEMPORARY TABLE IF EXISTS tmp_sle_payment_details;
        DROP TEMPORARY TABLE IF EXISTS tmp_sle_lien_settlements;
    ...
    WHERE (table_schema = DATABASE() AND table_type = 'BASE TABLE'
           AND table_name IN (
               ...
               'liens_LienSettlements','liens_SettlementPaymentDetails',
    ...
    ALTER TABLE tmp_sle_lf_parents
    ...
    WHERE UPPER(TRIM(COALESCE(lm.LM_IS_DELETED, 'N'))) <> 'Y'
      AND CAST(lm.LM_PROGRAM AS CHAR) = CAST(v_legacy_program AS CHAR)
      AND UPPER(TRIM(COALESCE(mc.LMC_STATUS, ''))) = 'A'
    GROUP BY mc.LMC_LM_ID;

    ALTER TABLE tmp_sle_amounts
        ADD PRIMARY KEY (LegacyLienId);
    ...
    ALTER TABLE tmp_sle_settlements
        ADD PRIMARY KEY (LegacyCaseId);
    ...
    ALTER TABLE tmp_sle_cases
        ADD COLUMN MetaBlock VARCHAR(1000) NULL,
        ADD COLUMN NotesAfter VARCHAR(4000) NULL,
        ADD PRIMARY KEY (LegacyCaseId),
        ADD UNIQUE INDEX UX_tmp_sle_cases_target (TargetCaseId);
    ...
    ALTER TABLE tmp_sle_liens
        ADD PRIMARY KEY (LegacyLienId),
        ADD INDEX IX_tmp_sle_liens_case (TargetCaseId);

    SELECT
        COUNT(*),
        COALESCE(SUM(BillingAmount), 0),
        COALESCE(SUM(PurchaseAmount), 0)
      INTO v_lien_count, v_total_billing, v_total_purchase
    FROM tmp_sle_liens;
    IF EXISTS (
    ...
    ALTER TABLE tmp_sle_facilities
        ADD PRIMARY KEY (LegacyFacilityId),
        ADD UNIQUE INDEX UX_tmp_sle_facilities_target (TargetFacilityId);
    ...
    ALTER TABLE tmp_sle_fac_persons
        ADD PRIMARY KEY (LegacyPersonId);
    ...
    ALTER TABLE tmp_sle_fac_links
        ADD PRIMARY KEY (LegacyLinkId),
        ADD INDEX IX_tmp_sle_fac_links_lien (TargetLienId);
    ...
    ALTER TABLE tmp_sle_med_codes
        ADD PRIMARY KEY (LegacyCodeId),
        ADD INDEX IX_tmp_sle_med_codes_lien (TargetLienId);
    ...
    ALTER TABLE tmp_sle_providers
        ADD PRIMARY KEY (LegacyLienId),
        ADD INDEX IX_tmp_sle_providers_lien (TargetLienId);
    ...
    -- -------------------------------------------------------------------------
    -- 18. Settlement staging
    -- Cash Received in the legacy dashboard is based on SLS_SETTLE_AMOUNT and
    -- SLS_SETTLE_DATE, not payment-detail check amounts.
    -- -------------------------------------------------------------------------
    DROP TEMPORARY TABLE IF EXISTS tmp_sle_lien_settlements;
    CREATE TEMPORARY TABLE tmp_sle_lien_settlements AS
    SELECT
        ...
        lien.TargetLienId,
        CASE
          WHEN NULLIF(TRIM(CAST(src.SLS_PAYMENT_NUMBER AS CHAR)), '') REGEXP '^[0-9]+$'
               AND CAST(TRIM(CAST(src.SLS_PAYMENT_NUMBER AS CHAR)) AS UNSIGNED)
                   BETWEEN 1 AND 2147483647
          THEN CAST(TRIM(CAST(src.SLS_PAYMENT_NUMBER AS CHAR)) AS UNSIGNED)
          ELSE ROW_NUMBER() OVER (
              PARTITION BY src.SLS_LIENS_ID
              ORDER BY src.SLS_ID)
        ...
        END AS Amount,
        CASE
          WHEN NULLIF(TRIM(CAST(src.SLS_SETTLE_DATE AS CHAR)), '') IS NULL THEN NULL
          WHEN TRIM(CAST(src.SLS_SETTLE_DATE AS CHAR))
               REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}'
          THEN DATE(STR_TO_DATE(
              LEFT(TRIM(CAST(src.SLS_SETTLE_DATE AS CHAR)), 10),
              '%Y-%m-%d'))
          WHEN TRIM(CAST(src.SLS_SETTLE_DATE AS CHAR))
               REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}'
          THEN DATE(STR_TO_DATE(
              SUBSTRING_INDEX(TRIM(CAST(src.SLS_SETTLE_DATE AS CHAR)), ' ', 1),
              '%c/%e/%Y'))
          ELSE NULL
        ...
        LEFT(CONCAT(
            ...
            1000) AS Note,
        src.SLS_CREATED AS CreatedAtUtc,
        src.SLS_UPDATED AS UpdatedAtUtc,
        SHA2(CONCAT_WS('|',
    ...
    IF EXISTS (
        SELECT 1
        FROM tmp_sle_lien_settlements
        WHERE Amount IS NULL
           OR (SettlementDateText IS NOT NULL AND SettlementDate IS NULL)
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'LSLTE-035 invalid lien settlement amount or date';
    END IF;

    ALTER TABLE tmp_sle_lien_settlements
        ADD PRIMARY KEY (LegacySettlementId),
        ADD INDEX IX_tmp_sle_settlements_lien (TargetLienId),
        ADD INDEX IX_tmp_sle_settlements_date (SettlementDate);
    SELECT COUNT(*), COALESCE(SUM(Amount), 0)
      INTO v_settlement_count, v_total_cash_received
    FROM tmp_sle_lien_settlements;

    -- Payment-detail rows are preserved for settlement history, but they do
    -- not drive the dashboard Cash Received metric. Deleted source rows are
    -- deliberately excluded.
    DROP TEMPORARY TABLE IF EXISTS tmp_sle_payment_details;
    CREATE TEMPORARY TABLE tmp_sle_payment_details AS
    SELECT
        src.SLSPD_ID AS LegacyPaymentDetailId,
        UUID() AS TargetPaymentDetailId,
        lien.TargetCaseId,
        lien.TargetLienId,
        CASE
          WHEN NULLIF(TRIM(CAST(src.SLSPD_PAYMENT_NUMBER AS CHAR)), '') REGEXP '^[0-9]+$'
               AND CAST(TRIM(CAST(src.SLSPD_PAYMENT_NUMBER AS CHAR)) AS UNSIGNED)
                   BETWEEN 1 AND 2147483647
          THEN CAST(TRIM(CAST(src.SLSPD_PAYMENT_NUMBER AS CHAR)) AS UNSIGNED)
          ELSE ROW_NUMBER() OVER (
              PARTITION BY src.SLSPD_LM_ID
              ORDER BY src.SLSPD_ID)
        END AS PaymentNumber,
        CASE
          WHEN src.RawAmount REGEXP '^-?[0-9]+(\\.[0-9]+)?$'
               AND CAST(src.RawAmount AS DECIMAL(30,8))
        ...
        END AS Amount,
        CASE
          WHEN NULLIF(TRIM(CAST(src.SLSPD_CHECK_DATE AS CHAR)), '') IS NULL THEN NULL
          WHEN TRIM(CAST(src.SLSPD_CHECK_DATE AS CHAR))
               REGEXP '^[0-9]{4}-[0-9]{2}-[0-9]{2}'
          THEN DATE(STR_TO_DATE(
              LEFT(TRIM(CAST(src.SLSPD_CHECK_DATE AS CHAR)), 10),
              '%Y-%m-%d'))
          WHEN TRIM(CAST(src.SLSPD_CHECK_DATE AS CHAR))
               REGEXP '^[0-9]{1,2}/[0-9]{1,2}/[0-9]{4}'
          THEN DATE(STR_TO_DATE(
              SUBSTRING_INDEX(TRIM(CAST(src.SLSPD_CHECK_DATE AS CHAR)), ' ', 1),
              '%c/%e/%Y'))
          ELSE NULL
        END AS PaymentDate,
        NULLIF(TRIM(CAST(src.SLSPD_CHECK_DATE AS CHAR)), '') AS PaymentDateText,
        NULLIF(TRIM(src.SLSPD_PAYOR), '') AS Payee,
        NULLIF(TRIM(src.SLSPD_CHECK_NUMBER), '') AS CheckNumber,
        LEFT(CONCAT(
            'legacyPaymentDetailId=', src.SLSPD_ID,
            '; legacyCaseId=', COALESCE(src.SLSPD_CASE_ID, ''),
            '; type=', COALESCE(src.SLSPD_TYPE, ''),
            '; status=', COALESCE(src.SLSPD_STATUS, ''),
            '; lienStatus=', COALESCE(src.SLSPD_LIEN_STATUS, ''),
            '; checkAmount=', COALESCE(src.SLSPD_CHECK_AMOUNT, ''),
            '; netProfit=', COALESCE(src.SLSPD_NET_PROFIT, '')),
            1000) AS Note,
        src.SLSPD_CREATED AS CreatedAtUtc,
        src.SLSPD_UPDATED AS UpdatedAtUtc,
        SHA2(CONCAT_WS('|',
            src.SLSPD_ID, src.SLSPD_CASE_ID, src.SLSPD_AMOUNT,
            src.SLSPD_CHECK_AMOUNT, src.SLSPD_CHECK_DATE,
            src.SLSPD_CHECK_NUMBER, src.SLSPD_TYPE, src.SLSPD_STATUS,
            src.SLSPD_PAYOR, src.SLSPD_NET_PROFIT,
            src.SLSPD_PAYMENT_NUMBER, src.SLSPD_LM_ID,
            src.SLSPD_LIEN_STATUS, src.SLSPD_AMOUNT_TO_SETTLE,
            src.SLSPD_CREATED, src.SLSPD_UPDATED, v_fingerprint),
            256) AS SourceHash
    FROM (
        SELECT
            p.*,
            COALESCE(
                NULLIF(TRIM(REPLACE(REPLACE(CAST(p.SLSPD_AMOUNT_TO_SETTLE AS CHAR), ',', ''), '$', '')), ''),
                NULLIF(TRIM(REPLACE(REPLACE(CAST(p.SLSPD_AMOUNT AS CHAR), ',', ''), '$', '')), ''),
                NULLIF(TRIM(REPLACE(REPLACE(CAST(p.SLSPD_CHECK_AMOUNT AS CHAR), ',', ''), '$', '')), ''),
                '0') AS RawAmount
        FROM `SL-CORE`.`SL_LIENS_SETTLEMENT_PAYMENT_DETAILS` p
        WHERE UPPER(TRIM(COALESCE(p.SLSPD_IS_DELETED, 'N'))) <> 'Y'
    ) src
    INNER JOIN tmp_sle_liens lien ON lien.LegacyLienId = src.SLSPD_LM_ID;

    IF EXISTS (
        SELECT 1
        FROM tmp_sle_payment_details
        WHERE Amount IS NULL
           OR (PaymentDateText IS NOT NULL AND PaymentDate IS NULL)
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'LSLTE-036 invalid settlement payment-detail amount or date';
    END IF;

    ALTER TABLE tmp_sle_payment_details
        ADD PRIMARY KEY (LegacyPaymentDetailId),
        ADD INDEX IX_tmp_sle_payment_lien (TargetLienId);
    SELECT COUNT(*) INTO v_payment_detail_count FROM tmp_sle_payment_details;
    ...
    -- -------------------------------------------------------------------------
    -- 19. Preflight report (p_apply = '0') — no permanent writes
    -- -------------------------------------------------------------------------
    IF NOT v_apply THEN
        DROP TEMPORARY TABLE IF EXISTS tmp_sle_payment_details;
        DROP TEMPORARY TABLE IF EXISTS tmp_sle_lien_settlements;
        ...
        SELECT
            ...
            v_settlement_count                          AS LienSettlementsToInsert,
            v_payment_detail_count                      AS SettlementPaymentDetailsToInsert,
            v_total_purchase                            AS TotalPurchaseAmountToInsert,
            v_total_billing                             AS TotalBillingAmountToInsert,
            v_total_cash_received                       AS TotalCashReceivedAmountToInsert;
        -- Normal exit: locks released, temp tables dropped above.
    ELSE
        -- -------------------------------------------------------------------
        -- 20. Apply: one SERIALIZABLE transaction for all writes
        -- -------------------------------------------------------------------
        ...
        -- Lien settlements drive the legacy-compatible Cash Received metric.
        ...
        SELECT
            ...
            PaymentNumber, Amount, SettlementDate, Status, Note, 0,
            COALESCE(CreatedAtUtc, UTC_TIMESTAMP(6)),
            COALESCE(UpdatedAtUtc, CreatedAtUtc, UTC_TIMESTAMP(6)),
            v_user_id, v_user_id
        FROM tmp_sle_lien_settlements;
        SET v_settlements_ins = ROW_COUNT();

        INSERT INTO liens_SettlementPaymentDetails (
            Id, TenantId, CaseId, LienId, PaymentNumber, Amount,
            PaymentDate, Payee, CheckNumber, Note, IsDeleted,
            CreatedAtUtc, UpdatedAtUtc, CreatedByUserId, UpdatedByUserId)
        SELECT
            TargetPaymentDetailId, v_tenant_id, TargetCaseId, TargetLienId,
            PaymentNumber, Amount, PaymentDate, Payee, CheckNumber, Note, 0,
            COALESCE(CreatedAtUtc, UTC_TIMESTAMP(6)),
            COALESCE(UpdatedAtUtc, CreatedAtUtc, UTC_TIMESTAMP(6)),
            v_user_id, v_user_id
        FROM tmp_sle_payment_details;
        SET v_payment_details_ins = ROW_COUNT();
        ...
        INSERT INTO liens_LegacyIdCrosswalks (
            Id, TenantId, SourceSystem, SourceTable, LegacyId, TargetEntity,
            TargetId, SourceHash, ImportRunId, CreatedAtUtc)
        SELECT UUID(), v_tenant_id, 'SL-CORE', 'SL_LIENS_SETTLEMENT',
               CAST(LegacySettlementId AS CHAR), 'LienSettlement',
               TargetSettlementId, SourceHash, v_core_run_id, UTC_TIMESTAMP(6)
        FROM tmp_sle_lien_settlements;

        INSERT INTO liens_LegacyIdCrosswalks (
            Id, TenantId, SourceSystem, SourceTable, LegacyId, TargetEntity,
            TargetId, SourceHash, ImportRunId, CreatedAtUtc)
        SELECT UUID(), v_tenant_id, 'SL-CORE', 'SL_LIENS_SETTLEMENT_PAYMENT_DETAILS',
               CAST(LegacyPaymentDetailId AS CHAR), 'SettlementPaymentDetail',
               TargetPaymentDetailId, SourceHash, v_core_run_id, UTC_TIMESTAMP(6)
        FROM tmp_sle_payment_details;
        ...
        SET Status = 'Completed', CompletedAtUtc = UTC_TIMESTAMP(6),
            SummaryJson = JSON_OBJECT(
                ...
                'lienSettlementsInserted', v_settlements_ins,
                'settlementPaymentDetailsInserted', v_payment_details_ins,
        ...
        SELECT
            ...
            v_settlements_ins                           AS LienSettlementsInserted,
            v_payment_details_ins                       AS SettlementPaymentDetailsInserted,
            v_lookups_seeded                            AS AccidentTypeLookupsSeeded,
            v_total_purchase                            AS TotalPurchaseAmountInserted,
            v_total_billing                             AS TotalBillingAmountInserted,
            v_total_cash_received                       AS TotalCashReceivedAmountInserted;
    END IF;
END$$
