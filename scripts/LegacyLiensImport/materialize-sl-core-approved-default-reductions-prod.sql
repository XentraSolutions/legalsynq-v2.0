-- Materializes canonical lien reductions for the production-approved 192-row SL-CORE
...
-- LS_LIENS production connection. Run preflight first and copy
...
BEGIN
    ...
    IF v_tenant_id IS NULL
       ...
       OR BINARY v_tenant_id <>
          BINARY '019f1a05-7459-7855-b46b-110a702e37a4'
       OR v_metadata_backfill_run_id IS NULL
       OR BINARY v_metadata_backfill_run_id <>
          BINARY '35cece1a-9e54-11f1-b823-12a7a8afef43'
    ...
    IF DATABASE() <> 'LS_LIENS' THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'LSLADR-003 target schema must be LS_LIENS';
    END IF;
