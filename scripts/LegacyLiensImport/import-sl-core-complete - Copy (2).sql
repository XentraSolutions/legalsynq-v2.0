...
BEGIN
    ...
    -- Keep lookup seeding and all staged reads in one SERIALIZABLE transaction.
    -- Preflight rolls this transaction back before returning, so p_apply = '0'
    -- never leaves a permanent lookup row behind.  Apply commits the same unit
    -- of work only after every import write and postcondition succeeds.
    SET TRANSACTION ISOLATION LEVEL SERIALIZABLE;
    START TRANSACTION;
    SET v_in_transaction = TRUE;

    -- -------------------------------------------------------------------------
    -- 4. Seed missing AccidentType lookup values (idempotent)
    ...
    IF EXISTS (
        ...
        HAVING SUM(BillingAmount) > 9999999999999999.99
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'LSLTE-029 case demand amount exceeds the target DECIMAL(18,2) limit';
    END IF;
    ...
    -- TaskNumber is unique per tenant.  A clean SL-CORE crosswalk does not
    -- prevent a manually-created task from using one of these deterministic
    -- legacy task numbers, so surface that conflict during preflight.
    IF EXISTS (
        SELECT 1
        FROM liens_ServicingItems t
        INNER JOIN (
            SELECT TaskNumber FROM tmp_sle_med_codes
            UNION ALL
            SELECT TaskNumber FROM tmp_sle_providers
            WHERE LegacyProviderCount = 1 AND TargetProviderId IS NOT NULL
        ) s ON s.TaskNumber = t.TaskNumber
        WHERE t.TenantId = v_tenant_id
    ) THEN
        SIGNAL SQLSTATE '45000'
            SET MESSAGE_TEXT = 'LSLTE-030 servicing task number collision with existing data';
    END IF;
    ...
    IF NOT v_apply THEN
        ...
        SELECT
            ...
            v_lookups_seeded                            AS AccidentTypeLookupsToSeed,
    ...
    ELSE
        -- -------------------------------------------------------------------
        -- 19. Apply: continue the SERIALIZABLE transaction opened before
        -- lookup seeding so every permanent change remains atomic.
        -- -------------------------------------------------------------------
