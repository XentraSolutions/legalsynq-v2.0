...
- **Port:** 5000 (dev)
- **Dev proxy:** `scripts/dev-proxy.js` — lightweight HTTP proxy on port 5000 that gates browser requests until Next.js (on internal port 3050) returns HTTP 200 for `/login`. Prevents the Next.js 15 + React 18 cold-compile race condition (`createMetadataComponents is not a function`) from ever reaching the browser. Auto re-gates if Next.js becomes unreachable. WebSocket upgrade passthrough for HMR.
