...
apps/
  ...
  services/
    ...
    tenant/                               ← TENANT-B01+B02+B03: Standalone Tenant service. 4-project DDD layout (Tenant.Domain / Tenant.Application / Tenant.Infrastructure / Tenant.Api). EF Core MySQL (tenant_db). Port :5005. Gateway: /tenant/**. In LegalSynq.sln. Auto-migrate on startup.
      ...
      Tenant.Application/                 ← DTOs/{TenantDtos,BrandingDtos,DomainDtos,TenantAdminDtos}.cs, Interfaces/{ITenantRepo,IBrandingRepo,IDomainRepo,ICapabilityRepo,ISettingRepo,ITenantSvc,IBrandingSvc,IDomainSvc,IResolutionSvc,IDocumentsAdapter,ITenantAdminService,IIdentityCompatAdapter}.cs, Services/{TenantService,BrandingService,DomainService,ResolutionService,TenantAdminService}.cs
      Tenant.Infrastructure/              ← TenantDbContext, Configurations/{Tenant,TenantBranding,TenantDomain}Configuration.cs, Repositories/{Tenant,Branding,Domain}Repository.cs, Services/{HttpDocumentsAdapter,HttpIdentityCompatAdapter}.cs, DependencyInjection.cs, Data/Migrations/
      Tenant.Api/                         ← Program.cs (JWT auth, AdminOnly policy, auto-migrate), Endpoints/{Tenant,Branding,Domain,Resolution,LogoAdmin,TenantAdmin}Endpoints.cs, Middleware/ExceptionHandlingMiddleware.cs; port 5005; ConnectionStrings:TenantDb; DocumentsService:InternalUrl=http://127.0.0.1:5006; IdentityService:InternalUrl=http://127.0.0.1:5001
