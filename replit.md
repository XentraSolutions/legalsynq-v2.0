...
- **Port:** 5000 (dev)
- **Dev proxy:** `scripts/dev-proxy.js` — lightweight HTTP proxy on port 5000 that (1) gates browser requests until Next.js (on internal port 3050) returns HTTP 200 for `/login`, and (2) intercepts any 5xx responses for HTML document requests and serves an auto-refreshing loading page instead. This prevents the Next.js 15 + React 18 cold-compile race condition (`createMetadataComponents is not a function` / `useReducer` null error) from ever reaching the browser — both on cold start AND during HMR module reloads. Auto re-gates if Next.js becomes unreachable (3+ consecutive connection errors). WebSocket upgrade passthrough for HMR. Note: patching `node_modules/next/dist/server/app-render/app-render.js` is ineffective because Next.js dev mode webpack-compiles its own server code into `.next/server/vendor-chunks/next.js`, bypassing the dist file entirely.
