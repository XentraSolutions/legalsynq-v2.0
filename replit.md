...
apps/
  ...
  services/
    ...
    tenant/                               ← TENANT-B01: Standalone Tenant service. 4-project DDD layout (Tenant.Domain / Tenant.Application / Tenant.Infrastructure / Tenant.Api). Entity: Tenant (Id, Code, DisplayName, LegalName, Status, Subdomain, LogoDocumentId, LogoWhiteDocumentId, TimeZone, timestamps). Enum: TenantStatus (Active/Inactive/Suspended). Field mapping from Identity identity_Tenants: Name→DisplayName, Code→Code, IsActive→Status, Subdomain→Subdomain, Logo*DocumentId preserved. EF Core MySQL (tenant_db, table: tenant_Tenants). REST API: GET /api/tenants, GET /api/tenants/{id}, POST /api/tenants, PUT /api/tenants/{id} — all require AdminOnly policy (PlatformAdmin role). Health: :5005/health. Gateway: /tenant/{**catch-all}. In LegalSynq.sln. Started by run-dev.sh and run-prod.sh. Migration: 20260422000000_InitialTenantSchema (tenant_Tenants table, unique indexes on Code and Subdomain). Production secret needed: ConnectionStrings__TenantDb.
      Tenant.Domain/                      ← Entities/Tenant.cs (DDD root with SetXxx mutators), Enums/TenantStatus.cs
      Tenant.Application/                 ← DTOs/TenantDtos.cs, Interfaces (ITenantRepository, ITenantService), Services/TenantService.cs
      Tenant.Infrastructure/              ← TenantDbContext, Configurations/TenantConfiguration.cs, Repositories/TenantRepository.cs, DependencyInjection.cs (AddTenantServices), Data/Migrations/
      Tenant.Api/                         ← Program.cs (JWT auth, AdminOnly policy, auto-migrate), Endpoints/TenantEndpoints.cs, Middleware/ExceptionHandlingMiddleware.cs; port 5005; ConnectionStrings:TenantDb
